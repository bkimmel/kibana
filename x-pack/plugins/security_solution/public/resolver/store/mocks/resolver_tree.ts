...
/**
 * Add/replace related event info (on origin) to any mock ResolverTree
 *  
 ...
 */
export function withRelatedEventsOnOrigin(treeToAddRelatedEventsTo: ResolverTree, relatedEventsToAddByCategoryAndType: Iterable<[string, string]>): ResolverTree {
  const events = [];
  ...
  for(const [category, type] of relatedEventsToAddByCategoryAndType){
    ...
    stats.events.total++;
    stats.events.byCategory[category] = stats.events.byCategory[category] ? (stats.events.byCategory[category] + 1) : 1;
  }
  return {
